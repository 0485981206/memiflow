      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => !open && closeDialog()}>
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <Label className="flex items-center gap-1.5"><Bot className="w-4 h-4" /> PDF-instructies voor de agent</Label>
              <Textarea
                value={form.pdf_instructies}
                onChange={(e) => setForm({ ...form, pdf_instructies: e.target.value })}
                placeholder="Beschrijf het formaat van de PDF, welke kolommen gebruikt worden, bijzonderheden per werknemer, enz."
