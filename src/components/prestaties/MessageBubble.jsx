...
const FunctionDisplay = ({ toolCall }) => {
  ...
  const statusConfig = {
    pending: { icon: Clock, color: 'text-slate-400', text: 'Wachten' },
    running: { icon: Loader2, color: 'text-slate-500', text: 'Bezig...', spin: true },
    in_progress: { icon: Loader2, color: 'text-slate-500', text: 'Bezig...', spin: true },
    completed: isError ? { icon: AlertCircle, color: 'text-red-500', text: 'Mislukt' } : { icon: CheckCircle2, color: 'text-green-600', text: 'Klaar' },
    success: { icon: CheckCircle2, color: 'text-green-600', text: 'Klaar' },
    failed: { icon: AlertCircle, color: 'text-red-500', text: 'Mislukt' },
    error: { icon: AlertCircle, color: 'text-red-500', text: 'Mislukt' }
    ...
    <div className="mt-2 text-xs">
      ...
      {expanded && !statusConfig.spin && (
        <div className="mt-1.5 ml-3 pl-3 border-l-2 border-slate-200 space-y-2">
          ...
          {parsedResults && (
            <div>
              <div className="text-xs text-slate-500 mb-1">Resultaat:</div>
